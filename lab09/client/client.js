...
const handleServerMessage = ev => {
    ...
    if (data.type === "login") {
        ...
        console.log(data);
        qs("#users-list > ul").innerHTML = data["active_users"].map(user => { return `<li>${user}</li>` }).join("");
        ...
        console.log(data);
        qs("#chat").innerHTML += `<span class="${data.username === username ? "right" : "left"}"><strong>${data.username}:</strong> ${data.text}</span>`;
    } else {
